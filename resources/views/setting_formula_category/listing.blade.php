                ...
                <div class="d-flex align-items-center">
                    ...
                    @if (auth()->user()->user_type_id == 1)
                        @can('formula_category_manage')
                            <a href="{{ route('setting_formula_category_add', ['tenant' => tenant('id')]) }}"
            ...
            {{-- @if (auth()->user()->user_type_id == 2) --}}
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-8">
                                <form method="POST" action="{{ route('setting_formula_category_listing', ['tenant' => tenant('id')]) }}">
                                    @csrf
                                    <div class="row">
                                        <div class="col-md-4">
                                                ...
                                                @if (auth()->user()->user_type_id == 1)
                                                    @can('formula_category_manage')
                                                        <td><span data-toggle='modal' data-target='#fulfill'
                                                                ...
                                                                class='fulfill'>
                                                                <a href="{{ route('setting_formula_category_edit', ['tenant' => tenant('id'), 'id' => $formula->setting_formula_category_id]) }}"
    ...
    <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form method="POST" action="{{ route('setting_formula_category_delete', ['tenant' => tenant('id')]) }}">
                    @csrf
