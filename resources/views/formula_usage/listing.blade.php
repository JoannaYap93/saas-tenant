	...
	<div class="col-12">
		<div class="page-title-box d-flex align-items-center justify-content-between">
			<h4 class="mb-0 font-size-18">
				<span class="mr-2">Formula Usage Listing</span>
                @can('formula_usage_manage')
                {{-- @if(auth()->user()->user_type_id == 2) --}}
                <a href="{{route('formula_usage_add', ['tenant' => tenant('id')])}}"
			...
			<div class="page-title-right">
				<ol class="breadcrumb m-0">
					<li class="breadcrumb-item">
						<a href="javascript: void(0);">Formula Company Usage</a>
					</li>
					...
					<table class="table table-nowrap">
						<thead class="thead-light">
							<tr>
                                ...
                                <th style="text-align: left;">Price Details</th>
						...
						<tbody>
							@if($records->isNotEmpty())
								...
								@php
										$status = $fu->formula_usage_status;
										$span_status = '';
										switch ($status) {
												case 'pending':
								...
								<tr>
									...
									<td>
                                        {{-- remark; there are several formula name not fetch or null, use ?? for temporary. json_decode is working --}}
										@if(auth()->user()->company_id == 0)
											<b>{{@$fu->company->company_name}}</b><br>
											<span>{{json_decode(@$fu->setting_formula->setting_formula_name)->en ?? @$fu->setting_formula->setting_formula_name}} - {{@$fu->formula_usage_type == 'man' ? 'Man' : 'Drone'}}</span><br>
										@else
											<b>{{json_decode(@$fu->setting_formula->setting_formula_name)->en ?? @$fu->setting_formula->setting_formula_name}}</b> - {{@$fu->formula_usage_type == 'man' ? 'Man' : 'Drone'}}<br>
										@endif
											<span style="font-style: italic;">{{@$fu->company_land->company_land_name}}</span><br>
											<span style="font-style: italic;">{{@$fu->company_land_zone_id != 0 ? $fu->company_land_zone->company_land_zone_name : "All Zone" }}</span><br>
									...
									<td align="left">
										@if(count(@$fu->formula_usage_item) > 0)
											@foreach(@$fu->formula_usage_item as $key => $fui)
												<b><a href="#" class="item_btn" data-toggle='modal' data-target='#formula_item_modal' id="item_btn_{{@$fui->formula_usage_item_id}}" data-id="{{@$fui->formula_usage_id}}" data-user-id="{{@$fu->user_id}}">{{json_decode(@$fui->raw_material->raw_material_name)->en ?? @$fui->raw_material->raw_material_name}}</a></b> - {{@$fui->formula_usage_item_total}} {{@$fui->raw_material->raw_material_value_unit == 'litre' ? "litre" : "kg"}}<br>
											@endforeach
										@else
										-
										@endif
									...
									<td align="left">
										@if(count(@$fu->formula_usage_product) > 0)
											<div class="table-responsive">
													<table class="table small-table table-striped mb-0">
															...
															<tbody>
																@foreach(@$fu->formula_usage_product as $key => $fup)
																<tr>
																	...
																	@foreach(@$company_pnl_sub_item as $key => $cpsi)
																		<td>
																			@php
																				$json_data = json_decode($fup->formula_usage_product_json);
																				$status = 0;
																				$data = '-';
																			@endphp
                                    ...
                                    <td>
                                        Total Price: RM{{number_format($fu->formula_usage_total_price,2)}} <br>
                                        Avg Price: RM{{number_format($fu->formula_usage_average_price_per_tree,2)}}
                                    </td>
									<td align="left">
										{!! $span_status !!}
										{{ date_format($fu->formula_usage_created, 'Y-m-d h:i A') }}<br>
										<span style="font-style: italic;">{{$fu->user->user_fullname}}</span>
									</td>
                                    @can('formula_usage_manage')
									<td align="left">
										@if($fu->formula_usage_status != 'deleted')
...
<script>
	$(document).ready(function(e) {
				...
				$('.item_btn').on('click', function() {
					...
					// console.log(user_id);
					$.ajax({
							url: "{{ route('ajax_find_formula_usage_item_details', ['tenant' => tenant('id')]) }}",
							method: "POST",
							data: {
									_token: "{{ csrf_token() }}",
							...
							success: function(e) {
									...
									if(e){
										e.result.forEach((item) => {
                                            name_eng = JSON.parse(item.raw_material_name);
											data += '<tr>';
											data += '<td align="center">'+ count +'</td>';
											data += '<td>'+ name_eng.en +'</td>';
											data += '<td>'+ item.formula_usage_item_qty +'</td>';
											data += '<td>'+ item.formula_usage_item_value +'</td>';
											data += '<td>'+ item.formula_usage_item_rounding +'</td>';
											data += '<td>'+ item.formula_usage_item_total +'</td>';
											data += '<td>'+ item.formula_usage_item_unit_price +'</td>';
											data += '<td>'+ item.formula_usage_item_total_price +'</td>';
											data += '</tr>';
											count++
										});
