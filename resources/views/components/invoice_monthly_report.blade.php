                        ...
                        <td class="h-auto" style="text-align: center; background-color: #e4e4e4;" colspan="2"><b>{{$product}}</b></td>
                            @foreach ($monthSel as $month_num => $month_name)
                                @if(isset($invoice['result'][$product][$month_num]['qty']) || isset($invoice['result'][$product][$month_num]['price']))
                                    <td style="text-align: center; {{ $month_num % 2 != 0 ? 'background-color: #ffffff;' : 'background-color: #e4e4e4;' }} "><a href="{{ route('invoice_daily_report', ['tenant' => tenant('id'),'year' => $search['year'],'month' => $month_num, 'company_id' =>  $search['company_id'], 'land_id' =>  $search['company_land_id'],'user_id' =>  $search['user_id' ]]) }}" target="_blank"> {{ ($invoice['result'][$product][$month_num]['qty'] > 0 ? number_format($invoice['result'][$product][$month_num]['qty'], 2, '.', '') : '-') }}</a></td>
                                    <td style="text-align: center; {{ $month_num % 2 != 0 ? 'background-color: #ffffff;' : 'background-color: #e4e4e4;' }} "><a href="{{ route('invoice_daily_report', ['tenant' => tenant('id'),'year' => $search['year'],'month' => $month_num, 'company_id' =>  $search['company_id'], 'land_id' =>  $search['company_land_id'],'user_id' =>  $search['user_id' ]]) }}" target="_blank"> {{ ($invoice['result'][$product][$month_num]['price'] ? number_format($invoice['result'][$product][$month_num]['price'], 2, '.', '') : '-') }}</a></td>
