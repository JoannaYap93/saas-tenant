<div>
  @if(!@$component)
      <p><b>Formula Usage Report - {{$monthName}} {{$search['year']}} ({{json_decode($settingFormulaCategoryName)->en}})</b></p>
  @endif
</div>
              ...
              <tr>
                  ...
                  @foreach ($companY as $ckey => $company)
                      <th style="min-width: 220px; text-align: center; {{ $count % 2 == 0 ? 'background-color: #ffffff;' : 'background-color: #e4e4e4;' }} border:1px solid #eee"
                          colspan='{{count($companyLand[$ckey]) * 3}}'>
                          {{ $company }}
                      </th>
                      @php
                        $count++;
                      @endphp
                  @endforeach
                  <th class="h-auto" style="text-align: center; font-weight: bold; background-color: #fffbaf; color:#000000" rowspan="2 "colspan="3">Total</th>
              </tr>
              <tr>
                @php
                  $count = 0;
