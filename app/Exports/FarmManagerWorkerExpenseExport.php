...
class FarmManagerWorkerExpenseExport implements FromView, WithEvents,ShouldAutoSize,WithColumnFormatting
{
    public function __construct($view, $company_expense_worker, $search)
    {
        $this->view = $view;
        $this->record = $company_expense_worker;
        $this->search = $search;
    }
    ...
    public function view(): View
    {
        return view($this->view, [
            'companyExpenseWorker' => $this->record,
            'search' => $this->search,
            'month' => Reporting::get_month_w_filter($this->search),
            'companies' => Company::get_company_for_daily_report($this->search),
            'managers' => User::get_farm_manager_name_by_company($this->search),
            'companyWorker' => CompanyExpenseWorker::get_company_expense_worker_id(),
        ]);
    }
